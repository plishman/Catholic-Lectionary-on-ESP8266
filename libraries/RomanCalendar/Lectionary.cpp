...
bool Lectionary::test(int number, String liturgical_year, String liturgical_cycle, bool *b_OT, bool *b_NT, bool *b_PS, bool *b_G) {
	...
	I2CSerial.print(F("Lectionary path is ")); I2CSerial.println(lect_path);
	...
	*b_G = false;
	...
	I2CSerial.print(F("testing Lectionary number ")); I2CSerial.println(String(lect_num));
	...
	if (!b_use_liturgical_year) {
		if (!(testFile(lect_path + lect_num + "/" + "G" + "/" + liturgical_cycle))) {
			I2CSerial.println(F("Lectionary::test() Malformed lectionary entry (No Gospel reading in either liturgical cycle or liturgical year)"));
			return false;
		}
	}
	...
	*b_G = true;
	...
	I2CSerial.print(F("Using Liturgical Year/Cycle ")); I2CSerial.println(lit_year_or_cycle);
	...
	*b_PS = testFile(lect_path + lect_num + "/" + "PS" + "/" + lit_year_or_cycle);
	...
	if (!*b_OT) I2CSerial.println(F("No OT reading"));
	if (!*b_NT) I2CSerial.println(F("No NT reading"));
	if (!*b_PS) I2CSerial.println(F("No PS reading"));
	
	return true;
}

bool Lectionary::testFile(String filename) {
	I2CSerial.print(F("testing file ")); I2CSerial.print(filename); I2CSerial.print(F(":"));
	
	char filenamestr[80];
	filename.toCharArray(filenamestr, 80);
	
	if (SD.exists(filenamestr)) {		
		I2CSerial.println(F("present"));
		return true;
	}
...
bool Lectionary::get(String liturgical_year, String liturgical_cycle, Lectionary::ReadingsFromEnum readings_from, int number, String* refs_text) {
	...
	I2CSerial.print(F("Looking for entry for liturgical year..."));
	file = /*_I18n->openFile*/SD.open(filename + "/" + liturgical_year, FILE_READ);

	if (!file.available()) {
		I2CSerial.print(F("not found. Looking for entry for liturgical cycle..."));
		file = /*_I18n->openFile*/SD.open(filename + "/" + liturgical_cycle, FILE_READ);
	}

	if (!file.available()) {
		I2CSerial.println(F("not found - Error: failed to get lectionary entry"));
		return false;
	} else {
		I2CSerial.println(F("done."));
	}
