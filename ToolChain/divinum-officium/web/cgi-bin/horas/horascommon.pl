...
# next day if $flag
sub getweek {
  my @weekdays_names = ("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday");
  ...
  if ($t >= $advent1) {
      ...
      my $christmas = date_to_days(25, 12 - 1, $year);
      #my @chd = days_to_date($christmas);

      if ($t == $christmas) {
        $dir_season = "Christmas";
        $dir_sub = "Day";
        $dir_day = $mnn; # Mass number (3 masses on Christmas Day)
        return '';
      }
    #}

    #if ($testmode eq "Season") {
      if ($t > $christmas && $t <= $christmas + 7) {
        $dir_season = "Christmas";
        if ($d[6] == 0) {
          $dir_day = $weekdays_names[$d[6]]; # Sunday in the Octave of Christmas
          return '';
        }
      }

      #if ($t > $christmas + 7 && $t < $christmas + 7 + 6 && $d[6] != 0) {
      #  $dir_season = "Christmas";
      #  $dir_day = "Feria";
      #  return '';
      #}
    ...
    if ($t < ($advent1 + 28)) {
      ...
      if ($month == (12 - 1) || $day < 25) {
        ...
        if ($testmode eq "Season") {
          ...
          my $advent3 = $advent1 + 14;
          if ($t == ($advent3 + 3) || $t == ($advent3 + 5) || $t == ($advent3 + 6)) {
            $dir_sub = "Ember"; # Advent Embertide
          }
          else {
            $dir_sub = $n;  
          }
  ...
  # PLL 24-06-2020
  # Holy Name
  my $newyearsday = date_to_days(1, 1 - 1, $year);  
  my @nyd = days_to_date($newyearsday);

  if ($testmode eq "Saint") {
    my $holyname = $newyearsday + (6 - (($nyd[6] + 6) % 7));
    if ($holyname == $newyearsday || $holyname == $newyearsday + 5 || $holyname == $newyearsday + 6) {
      $holyname = $newyearsday + 1;
    }
  ...
  # PLL 24-06-2020
  if ($testmode eq "Saint") {
    if ((leapyear($year) && $dnn eq "25/2") || (!leapyear($year) && $dnn eq "24/2")) {
      # Saint Matthias Day (moves to 25 Feb on leap years, from 24 Feb)
      $dir_day = "StMatthias";
      return '';
    }
  }
  ...
  if ($testmode eq "Saint") {
    if ((leapyear($year) && $dnn eq "28/2") || (!leapyear($year) && $dnn eq "27/2")) {
      # StGabrielOfOurLadyOfSorrows (moves to 28 Feb on leap years, from 27 Feb)
      $dir_day = "StGabrielOfOurLadyOfSorrows";
      return '';
    }
  }

  if ($testmode eq "Saint") {
    if ($dnn eq "11/2") { # For the Faithful Departed
      $dir_season = $mmm;
      $dir_sub = $ddd;
      $dir_day = $mnn; # Mass number (3 masses on 2 November)
      return '';
    }
  }
...
#  }
  ...
  if ($month == 1 && $day < 7) {
    # PLL 24-06-2020
    #if ($testmode eq "Saint") {
    #  if ($t > $newyearsday && $t < $newyearsday + 6 && $d[6] != 0 && $d[6] != 6) {
    #    $dir_season = "Christmas";
    #    $dir_day = "Feria";
    #  }
    #}
  ...
  my $ordtime = date_to_days(6, 1 - 1, $year);
  ...
  if ($t < $easter - 63) {
    ...
    # PLL 18-06-2020
    if ($testmode eq "Season") {
      if ($n == 1 && $d[6] == 0) { # Feast of the Holy Family
        $dir_season = "Christmas";
        $dir_day = "HolyFamily";
      }
      else {
  ...
  if ($t < $easter - 56) { 
    # PLL 18-06-2020
    if ($testmode eq "Season") {
      $dir_season = "Septuagesima";
      $dir_day = $weekdays_names[$d[6]];
    }
    # PLL 18-06-2020

    return getname("Quadp1"); 
  }
  
  if ($t < $easter - 49) { 
    # PLL 18-06-2020
    if ($testmode eq "Season") {
      $dir_season = "Sexagesima";
      $dir_day = $weekdays_names[$d[6]];
    }
    # PLL 18-06-2020

    return getname("Quadp2"); 
  }
  
  if ($t < $easter - 42) { 
    # PLL 18-06-2020
    if ($testmode eq "Season") {
      $dir_season = "Quinquagesima";
      $dir_day = $weekdays_names[$d[6]];
    }
    # PLL 18-06-2020

    return getname("Quadp3"); 
  }
  ...
  if ($t < ($easter)) {
    ...
    if ($testmode eq "Season") {
      ...
      elsif ($d == $easter - 46 + 7 || $d == $easter - 46 + 9 || $d == $easter - 46 + 10) {
        $dir_season = "Lent";
        $dir_sub = "Ember";
        $dir_day = $weekdays_names[$d[6]];
      }
  ...
  if ($t < ($easter + 56)) {
    ...
    # PLL 18-06-2020
    if ($testmode eq "Season") {
      if ($n == 5) { # Ascension Thursday, Rogations, Feria
        #$dir_season = "Ascension";
        $dir_season = "Easter";
        $dir_sub = "5";
        $dir_day = $weekdays_names[$d[6]]; #Thursday
                
       # if ($d[6] == 1 || $d[6] == 2) { # Monday, Tuesday and Wednesday before Ascension Thursday are the Minor Rogations (mass is the same (?))
       #   #$dir_season = "Easter";
       #   #$dir_sub = $n;
       #   #$dir_day = "Rogation";
       #   $dir_subsub = "Rogation";
       ...
       # 
       # #$dir_season = "Ascension";
       # if ($d[6] == 3) { # Ascension Vigil (Wednesday before Ascension Thursday)
       #   $dir_day = "Vigil"; #Wednesday before Ascension Thursday
       # }
       # elsif ($d[6] == 4) { # Ascension Thursday
       #   $dir_day = $weekdays_names[$d[6]]; #Thursday
       # }
       # elsif ($d[6] == 5) { # Friday after Ascension, Feria
       #   $dir_day = "Feria"; 
       # }
       # elsif ($d[6] == 6) { # Saturday after Ascension, Feria
       #   $dir_day = $weekdays_names[$d[6]]; 
       # }
      }
      elsif ($n == 6) {
       ...
       # if ($d[6] == 0) { # Sunday after Ascension Thursday
       #   $dir_season = "Ascension";
       #   $dir_day = $weekdays_names[$d[6]];
       # }
       # elsif ($d[6] == 6) { # Vigil of Pentecost
       #   $dir_season = "Pentecost";
       #   $dir_day = "Vigil";
       # }
      }
      elsif ($n == 7) { # Pentecost week
        $dir_season = "Pentecost";
        if ($d[6] != 0) {
          $dir_sub = "Whit";
        }
  ...
  if ($n < 23) { 
    ...
    if ($testmode eq "Season") {
      ...
      if ($n == 1) {
      ...
      elsif ($n == 2 && $d[6] == 5) { # Friday of the week after Corpus Christi is Sacred Heart
        ...
        my @fs = days_to_date($first_september);
        my $third_sunday_in_september = 14 + $first_september + (6 - (($fs[6] + 6) % 7));

        if ($t == ($third_sunday_in_september + 3) || $t == ($third_sunday_in_september + 5) || $t == ($third_sunday_in_september + 6)) {
          $dir_sub = "Ember"; # Michaelmas Embertide
        }
        else {
          $dir_sub = $n;
        }
    ...
    return getname(sprintf("Pent%02i", $n)); 
  }

  my $wdist = floor(($advent1 - $t + 6) / 7);
  if ($wdist < 2 && $t < $advent1) { # drops into here on Christmas Day? 2019
    # PLL 18-06-2020
    if ($testmode eq "Season" || $d[6] == 0) {
      $dir_season = "Pentecost";
      $dir_sub = "Last";
      $dir_day = $weekdays_names[$d[6]];
    }
...
sub days_to_date {
  ...
  if ($days < -141427) { 
    print "days_to_date() $ddd-$mmm-$yyyy [$dtt] ";
    error("Date before the Gregorian Calendar!"); 
  }
  my @d = splice(@d, @d);
  $d[0] = 0;
  $d[1] = 0;
...
sub date_to_days {
  ...
  $ret += ($d - 1);
  if ($ret < -141427) { 
    print "date_to_days() $ddd-$mmm-$yyyy [$dtt]";
    error("Date before the Gregorian Calendar!"); 
  }
  return $ret;
}
