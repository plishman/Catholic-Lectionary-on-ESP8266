  ...
  if ($Ck) {
    ...
    if ($Propers) {
      if ($lang eq "Latin") {
        ...
        if ($mass_heading eq "") {
          $fnn_la = $fnn_la_last;
          $hnn_la = $hnn_la_last + 1;
          $appendflag_la = 1;
        }

        write_ordo($text, $fnn_la, $hnn_la, $lang, $dnn, 0, $appendflag_la); 
        #$fnn_la += 1;
        #$hnn_la = 0;
      }
      else {
        my $appendflag_alt = 0;
        if ($mass_heading_alt eq "") {
          $fnn_alt = $fnn_alt_last;
          $hnn_alt = $hnn_alt_last + 1;
          $appendflag_alt = 1;
        }
        write_ordo($text, $fnn_alt, $hnn_alt, $lang, $dnn, 0, $appendflag_alt); 
        #$fnn_alt += 1;
        #$hnn_alt = 0;
      }
    }
...
#      if ($Propers) {
#        if ($lang eq "Latin") {
#          write_ordo($item, $fnn_la, $hnn_la, $lang1, $dnn);  
#          $fnn_la += 1;
#          #$hnn_la = 0;
#        }
#      }
...
#        #PLL 29-06-2020
#        if ($Propers) {
#          write_ordo($item, $fnn_alt, $hnn_alt, $lang, $dnn); 
#          $fnn_alt += 1;
#          #$hnn_alt = 0;
#        }
#        #PLL 29-06-2020
